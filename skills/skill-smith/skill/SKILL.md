---
name: smith
description: Swift architecture validation and general development patterns. Project structure analysis, dependency management, build optimization, universal Swift patterns. For TCA-specific guidance, use "tca" skill. Use for "Is my architecture good?", "project structure", "dependency injection", "Swift patterns", "build optimization", "code organization", "Swift Testing", concurrency patterns.
...
> **Transition status:** Heuristic bash analyzers (testability score, extraction suggestions, dependency graphs) were retired from the skill bundle and archived under `../archived-smith-scripts/` pending TODO `SV-HEURISTICS-UPGRADE` to rebuild them as SmithValidation rules.
...
"Use tca skill for TCA patterns and reducer guidance"
"Use smith-platforms for visionOS"
"Use smith + tca skills together for comprehensive analysis"
```

## 🔄 **Skill Integration - When to Use Which**

**smith-skill handles:**
- ✅ **General Swift architecture** and project structure
- ✅ **Build optimization** and tool integration
- ✅ **Universal patterns** (dependencies, testing, concurrency)
- ✅ **smith-validation** orchestration

**For TCA-specific questions, use the dedicated tca skill:**
- 🎯 **TCA patterns** and modern 1.23.0+ best practices
- 🎯 **@Shared state** management and discipline
- 🎯 **TCA testing** with Swift Testing framework
- 🎯 **TCA navigation** and SwiftUI integration
- 🎯 **TCA anti-patterns** and deprecated API migration
...
## 🎯 **TCA Questions? Use the tca Skill**

For TCA-specific expertise, Smith automatically defers to the dedicated **tca skill**:

**TCA skill auto-load keywords:**
- "TCA", "@Reducer", "@ObservableState", "@Shared", "ComposableArchitecture"
- "TCA patterns", "TCA testing", "TCA navigation"
- "WithViewStore", "ViewStore", "@Bindable" (modern TCA)
...
**🎯 For TCA Questions: Use the tca Skill**
- ✅ **TCA patterns** (Swift Composable Architecture) → **tca skill**
- ✅ **@Reducer**, **@ObservableState**, **@Shared** → **tca skill**
- ✅ **SwiftUI navigation** with TCA (.sheet, .scope) → **tca skill**
- ✅ **TCA compilation errors** → **tca skill**
- ✅ **WithViewStore**, **ViewStore**, **@Bindable** migration → **tca skill**
- ✅ **TCA testing** with Swift Testing → **tca skill**
- ✅ **TCA anti-patterns** and deprecated APIs → **tca skill**
...
- ✅ **TCA anti-pattern detection** → **smith-validation** for automated detection + **tca skill** for fixes
- ✅ **Feature extraction** strategies → **tca skill** for patterns and best practices

**Examples:**
- "How do I add dependency injection to my Swift code?" → smith-core
- "My TCA reducer won't compile, what's wrong?" → **tca skill**
...
**Smith works with complementary skills for comprehensive coverage:**

### **tca skill** - TCA Expertise
✅ **TCA patterns** and modern 1.23.0+ best practices
✅ **@Shared state** management and discipline
✅ **TCA testing** with Swift Testing framework
✅ **TCA anti-patterns** and deprecated API migration
...
```
General Swift architecture → smith skill
TCA patterns & guidance    → tca skill
API/WWDC documentation     → sosumi skill
Multiple needs?           → Use appropriate combination
```

### **Integration Examples**
**Need multiple skills (automatically triggered):**
- "How do I implement GroupActivities with TCA?" → **tca skill** + **sosumi skill**
- "My SwiftUI app with TCA has performance issues" → **smith skill** + **tca skill**
- "Build TCA app with proper Apple platform integration" → **smith skill** + **tca skill** + **sosumi skill**
...
smith validate --tca-only [path] \
  ...
  --tca-max-examples 5
```
- Rule 1.1: Monolithic features (State > 15 props, Actions > 40 cases) — configurable via flags
- Rule 1.2: Proper dependency injection
- Rule 1.3: Code duplication
- Rule 1.4: Unclear organization
- Rule 1.5: Tightly coupled state
- Swift Testing–style output or detailed ASCII summary (`--tca-output detailed`)
